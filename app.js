...
app.post('/register', async (req, res) => {
    const { nombre, email, cedula, contraseña } = req.body; // Asegúrate de usar el nombre correcto para la contraseña
    if (!nombre || !email || !cedula || !contraseña) {
        return res.send('Todos los campos son obligatorios.');
    }

    try {
        // Genera la sal (salt) para el cifrado de la contraseña
        const saltRounds = 10;
        const hashedPassword = await bcrypt.hash(contraseña, saltRounds); // Pasa la contraseña y la sal como argumentos
        
        // Continúa con el proceso de registro utilizando la contraseña cifrada
        ...
        return res.send('Error al registrar el usuario. Por favor, inténtalo de nuevo.');
    }
});

app.get('/login', (req, res) => {
    res.render('login', { mensajeError: req.session.mensajeError });
    ...
    db.query('SELECT * FROM usuarios WHERE email = ?', [email], async (err, results) => {
        ...
        if (results.length > 0) {
            ...
            if (await bcrypt.compare(contraseña, user.contraseña)) {
                ...
                // Si la contraseña es incorrecta, configuramos el mensaje de error correspondiente
                req.session.mensajeError = 'Contraseña incorrecta, por favor intenta de nuevo.';
                res.redirect('/login');
            }
        } else {
            // Si el usuario no se encuentra, configuramos el mensaje de error correspondiente
